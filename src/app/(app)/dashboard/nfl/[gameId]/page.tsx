...
export default async function NflGamePage({ params }: { params: Promise<{ gameId: string }> }) {
  ...
  return (
    <main style={{ maxWidth: 1120, margin: '40px auto', padding: 16 }}>
      <Link href="/dashboard/nfl" style={{ color: '#8fc7ff' }}>
        ← Back to NFL Week {result.week}
      </Link>
      <header style={{ ...cardStyle, marginTop: 16, marginBottom: 24 }}>
        <p style={{ margin: 0, opacity: 0.75 }}>
          {result.season} Week {result.week} · {game.scheduledStartDate.toLocaleDateString()}
        </p>
        <h1 style={{ marginBottom: 8 }}>
          {game.awayTeam} at {game.homeTeam}
        </h1>
        <p style={{ color: '#f6c85f' }}>
          {nflResultsDescription(game.decisionUse ?? result.decisionUse)}. Model probabilities are
          not sportsbook quotes, picks, or evidence of betting edge.
        </p>
        <nav
          style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}
          aria-label="Game result sections"
        >
          <a href="#outlook" style={{ color: '#8fc7ff' }}>
            Game outlook
          </a>
          <a href="#lines" style={{ color: '#8fc7ff' }}>
            Model lines
          </a>
          <a href="#players" style={{ color: '#8fc7ff' }}>
            Player projections ({result.playerProjections.length})
          </a>
        </nav>
      </header>
      ...
      <section id="outlook" style={{ scrollMarginTop: 16 }}>
        ...
        >
          <div style={cardStyle}>
            <strong>{game.awayTeam} win</strong>
            <div style={{ fontSize: 28 }}>{pct(game.awayWinProbability)}</div>
          </div>
          <div style={cardStyle}>
            <strong>{game.homeTeam} win</strong>
            <div style={{ fontSize: 28 }}>{pct(game.homeWinProbability)}</div>
          </div>
          <div style={cardStyle}>
          ...
          <div style={cardStyle}>
            ...
            <div style={{ fontSize: 28 }}>{game.projectedTotalCalibrated.toFixed(1)}</div>
          </div>
        </div>
        <p style={{ opacity: 0.8 }}>
          Total range (P10/P50/P90): {game.totalP10.toFixed(1)} / {game.totalP50.toFixed(1)} /{' '}
          {game.totalP90.toFixed(1)} · {game.provisional ? 'Provisional' : 'Declared ready'}
        </p>
      </section>
      ...
      <section id="players" style={{ marginTop: 32, scrollMarginTop: 16 }}>
        ...
        <p style={{ opacity: 0.8 }}>
          {result.playerProjections.length} players from this matchup, derived from the same{' '}
          {result.trialsPerGame.toLocaleString()} simulation trials.
        </p>
        {result.playerProjections.map((player) => (
          <details key={player.id} style={{ ...cardStyle, marginBottom: 12 }}>
            ...
            <p style={{ opacity: 0.8 }}>
              Mean: {player.passingYards.toFixed(1)} pass yds · {player.rushingYards.toFixed(1)}{' '}
              rush yds · {player.receivingYards.toFixed(1)} rec yds · {player.receptions.toFixed(1)}{' '}
              receptions · {player.totalTouchdowns.toFixed(2)} TD
            </p>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
